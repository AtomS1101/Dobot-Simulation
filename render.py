...
class Render:
	...
	def _convCrd(self, viewMode: str, point: tuple) -> tuple:
		if viewMode == "angled":
			x = point[0] + point[1] * math.cos(math.radians(settings.Y_AXIS_ANGLE)) + settings.ANGLED_VIEW_BEGIN[0] + 50
			y = point[2] + point[1] * math.sin(math.radians(settings.Y_AXIS_ANGLE)) + settings.ANGLED_VIEW_BEGIN[1] + 50
		elif viewMode == "front":
			xArea = settings.FRONT_VIEW_END[0] - settings.FRONT_VIEW_BEGIN[0]
			yArea = settings.FRONT_VIEW_END[1] - settings.FRONT_VIEW_BEGIN[1]
			x = xArea / (settings.FRONT_CRD_END[0] - settings.FRONT_CRD_BEGIN[0]) * point[0] + (xArea / 2) + settings.FRONT_VIEW_BEGIN[0]
			y = yArea / (settings.FRONT_CRD_END[1] - settings.FRONT_CRD_BEGIN[1]) * point[1] + settings.FRONT_VIEW_BEGIN[1] + 50
		elif viewMode == "top":
			xArea = settings.TOP_VIEW_END[0] - settings.TOP_VIEW_BEGIN[0]
			yArea = settings.TOP_VIEW_END[1] - settings.TOP_VIEW_BEGIN[1]
			x = xArea / (settings.TOP_CRD_END[0] - settings.TOP_CRD_BEGIN[0]) * point[0] + (xArea / 2) + settings.TOP_VIEW_BEGIN[0]
			y = yArea / (settings.TOP_CRD_END[1] - settings.TOP_CRD_BEGIN[1]) * point[1] + (yArea / 2) + settings.TOP_VIEW_BEGIN[1]
		else:
			return point
		return (x, y)
	...
	def _showInfo(self, info: list):
		...
		self._canvas.pen("info").showText("θ1", (150, 220))
		self._canvas.pen("info").showText("θ2", (150, 190))
	...
	def render(self, angle1: float, angle2: float, direction: float):
		...
		# Angled View
		self._canvas.pen("angled").color((0, 0, 0)).size(16).align("right").clear()
	...
	def drawAxis(self):
		self._canvas.pen("axis").clear()
		self._canvas.pen("axis")\
				.color((0, 0, 0))\
				.width(1)\
				.size(16)\
				.align("right")
		self._canvas.pen("axis").drawSquare(settings.ANGLED_VIEW_BEGIN, settings.ANGLED_VIEW_END)
		self._canvas.pen("axis").drawSquare(settings.TOP_VIEW_BEGIN,    settings.TOP_VIEW_END)
		self._canvas.pen("axis").drawSquare(settings.FRONT_VIEW_BEGIN,  settings.FRONT_VIEW_END)
		...
		self._canvas.pen("axis").color((230, 230, 230))
		for i in range(settings.FRONT_CRD_BEGIN[0] + settings.AXIS_STEPS, settings.FRONT_CRD_END[0] - settings.AXIS_STEPS, settings.AXIS_STEPS):
			self._canvas.pen("axis").drawLine(self._convCrd("front", (i, settings.FRONT_CRD_BEGIN[1])), self._convCrd("front", (i, settings.FRONT_CRD_END[1]))) # draw vertical lines
		for i in range(settings.FRONT_CRD_BEGIN[1] + settings.AXIS_STEPS, settings.FRONT_CRD_END[1] - settings.AXIS_STEPS, settings.AXIS_STEPS):
			self._canvas.pen("axis").drawLine(self._convCrd("front", (settings.FRONT_CRD_BEGIN[0], i)), self._convCrd("front", (settings.FRONT_CRD_END[0], i))) # draw horizontal lines
		self._canvas.update()
